...
logger = setup_logger("STT-Voice-Worker")
...
async def handle_stt_transcribe(req: STTTranscribeRequest) -> STTTranscribeResponse:
    logger.info(f"[STT Worker] Received transcription request {req.request_id} (format={req.audio_format}, lang={req.language})")
...
async def handle_stt_load(req: ModelLoadRequest):
    logger.info(f"[STT Worker] Loading STT model file: {req.filename}")
...
@broker.subscriber("overlay.models.stt.unload")
async def handle_stt_unload(req: ModelLoadRequest):
    logger.info(f"[STT Worker] Unloading STT model")
    engine.active_filename = None
    engine.model = None
    await broker.publish(
        {
            "type": "MODEL_STATUS",
...
async def on_startup():
    logger.info("STT Worker started successfully. Initializing whisper.cpp models...")
